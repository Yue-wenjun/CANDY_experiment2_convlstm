...
class MovingMnistTestSetGenerator():
  ...
  def __init__(self, mnistDataset='mnist.h5', mode='standard', background='zeros', num_frames=20, save_dir="./mnist_test", batch_size=1000, image_size=64, num_digits=2, step_length=0.1):
    self.mode_ = mode
  ...
  def GenerateTestSet(self):
    ...
    np.save(self.save_dir + "/mnist_test_500.npy", dataset)
    """
    np.save(self.save_dir + "/mnist_test1.npy", dataset[:2500,:,:,:,:])
    np.save(self.save_dir + "/mnist_test2.npy", dataset[2500:5000,:,:,:,:])
    np.save(self.save_dir + "/mnist_test3.npy", dataset[5000:7500,:,:,:,:])
    np.save(self.save_dir + "/mnist_test4.npy", dataset[7500:,:,:,:,:])
    """
    return True
...
generator = MovingMnistTestSetGenerator()
